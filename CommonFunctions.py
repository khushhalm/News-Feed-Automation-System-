...
def extractrssresponse(response=None, timeline_start_date=1, timeline_start_hour=0):
    ...
    for each_story in stories:
        ...
        if checkifdatetime_within_timelinelimit(input_date_time=story_date_time,
            ...
            story_details['title'] = each_story.get("title")
            story_details['published_date'] = str(story_date_time)
            story_details['summary'] = each_story.get("summary")
            story_details['articlebody'] = each_story.get("story")
            article_links[each_story.get('link')] = story_details
        else:
            break
...
def find_module(module_name=None, function_name=None):
    try:
        fp, path, desc = imp.find_module(module_name, ['/home/hp/NFA-System/Modules/NewsPaper_Plugins'])
        module_desc={'fp': fp, 'path': path, 'desc': desc}
        return module_desc
    except Exception as e:
